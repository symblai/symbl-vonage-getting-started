...
# CLIENT component
pushd "${MY_DIR}/opentok-web-samples/Vue-Basic-Video-Chat" || exit 1
    ...
    if [[ "${GET_VALUE}" == "" ]]; then
        heroku config:set --app="${CLIENT_NAME}" VUE_APP_APP_ID="${APP_ID}" >/dev/null 2>&1
    fi
    GET_VALUE=$(heroku config:get --app="${CLIENT_NAME}" APP_SECRET)
    if [[ "${GET_VALUE}" == "" ]]; then
        heroku config:set --app="${CLIENT_NAME}" VUE_APP_APP_SECRET="${APP_SECRET}" >/dev/null 2>&1
    fi
    GET_VALUE=$(heroku config:get --app="${CLIENT_NAME}" SAMPLE_SERVER_BASE_URL)
    if [[ "${GET_VALUE}" == "" ]]; then
        heroku config:set --app="${CLIENT_NAME}" VUE_APP_SAMPLE_SERVER_BASE_URL="https://${SERVER_NAME}.herokuapp.com" >/dev/null 2>&1
    fi
    ...
    heroku config:set --app="${CLIENT_NAME}" PROJECT_PATH=Vue-Basic-Video-Chat
